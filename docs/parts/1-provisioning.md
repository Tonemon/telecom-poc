...
**How the `suspend` works (two enforcement legs):**

1. **Live detach (leg 1):** the server first updates the *live* document with `request_cancel_location: true` (plus `subscriber_status`/`operator_determined_barring`). Mongo runs as a single-node replica set specifically so the HSS's `use_mongodb_change_stream` watcher can see that update and fire a genuine Diameter S6a **Cancel-Location-Request** (`Cancellation-Type = SUBSCRIPTION_WITHDRAWAL`). The MME reacts exactly as it would to a real HSS: it sends the UE a NAS **Detach-Request** and tears down its bearer. The HSS also sets `CLR-Flags = REATTACH_REQUIRED` on this cancellation type, so the UE's own NAS stack retries attaching almost immediately. A real telco doesn't leave a barred device silently dark, it tells the device to retry now so the network can hand back an explicit reject.
2. **Stash-and-restore (leg 2):** the server then **moves** the subscriber document out of the live `subscribers` collection into a `suspended_subscribers` holding collection (all data + keys preserved, `subscriber_status` stamped `1` as honest metadata), so the HSS stops answering for it. This is what actually rejects the reattach attempt leg 1 just triggered and any later attach attempt too. `resume` moves the document back and clears the leg-1 flags; it sends nothing to the MME. **Nothing is deleted** and both legs are fully reversible. Leg 2 stays because the MME still doesn't check `subscriber_status` on attach, where leg 1 alone would detach an active session but not stop the UE's own immediate reattach from succeeding. `make test-provisioner-lifecycle` proves both legs end-to-end: attach + ping, then `suspend` an **already-attached** UE and measure real packet loss across the outage (leg 1 is what a stash-only test can never exercise, since it requires the UE to already be live when suspend fires; the interface can hold its address throughout, so packet loss is the reliable signal, not the address disappearing), then `resume` and confirm the UE regains service on its own: its NAS stack retries attaching immediately per `REATTACH_REQUIRED`, and if that lands before leg 2 clears it's rejected. The UE then falls back to periodically retrying (3GPP's `T3402` timer, ~20s in this lab) until an attempt succeeds, no container recreate involved. Then a second suspend/attach-attempt cycle proves leg 2 rejects fresh attach attempts too.

**Why this is the right foundation for the roadmap:** enforcement lives at the **home HSS** via the standard S6a path that **roaming** also uses (a visited MME queries the home HSS; `USER_UNKNOWN` blocks the roamer regardless of whether the visited MME implements barring, and CLR detaches it live regardless of which MME is serving it). It is orthogonal to **X2 handover** (a RAN-side procedure). One future roaming-specific enhancement remains:

- patch the vendored Open5GS **MME to honor `Subscriber-Status`** for the authentic on-the-wire ODB-reject semantics (leg 2 already gives the same net effect without this).
