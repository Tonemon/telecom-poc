...
This performs the following from a clean state automatically: build, start all services, provision the test subscriber, wait for the UE to attach, ping `8.8.8.8` through the UPF. From the repo root:
...
As a middle ground between the one-shot `make 4g-auto` and the fully by-hand walkthrough in §5.3, the deployment is also split into an **infra** stage, a **subscribers** stage, and a **device** stage. Each prints what it is doing and which containers it starts/stops. These sit *alongside* the manual steps, not instead of them:

```bash
make 4g-infra             # EPC core + eNB only — the operator network, NO subscribers, NO UE
make 4g-demo-subscribers  # register 3 demo subscribers with the telcoctl client
make 4g-device            # bring up the UE, wait for attach, ping the internet
...
`4g-infra` deliberately provisions **nothing**, it just stands up the network, the way a real operator's EPC exists before any SIM is sold. Subscribers are then created through the `telcoctl` client (`4g-infra` prints the exact command to run). `4g-demo-subscribers` is a convenience that registers three: the first is the soft-UE's own SIM (its keys match `ue.conf`, so `4g-device` can attach), and two more are batch-issued via `telcoctl issue-sim` with server-generated keys to show the client managing a real roster.
